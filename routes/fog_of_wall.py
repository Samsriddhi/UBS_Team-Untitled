...
This improved solution combines multiple strategies:
1. Preplanned scanning pattern with overlapping coverage
2. Adaptive exploration for unknown areas
3. Wall boundary tracking
4. Crow coordination to minimize redundant moves
5. Dynamic strategy switching based on progress

Key improvements:
- Better handling of edge cases and malformed inputs
- More efficient scanning pattern with overlap handling
- Improved pathfinding with obstacle avoidance
- Better crow coordination and task assignment
- Recovery mechanisms for stuck situations
...
# Strategy constants
SCAN_RADIUS = 2  # 5x5 scan means radius 2
MIN_USEFUL_UNKNOWN = 3  # Minimum unknown cells for a scan to be "useful"
MAX_STUCK_MOVES = 5  # Max moves before considering a crow "stuck"
...
def parse_move_result(mr) -> Optional[Tuple[int, int]]:
    ...
    with 'x' and 'y' keys. Returns a tuple (x, y) or `None` if parsing fails.
    """
...
def update_known_from_scan(center: Tuple[int, int], scan: List[List[str]], L: int,
                           known: Dict[Tuple[int, int], str]) -> int:
    """
    Update the `known` map based on a 5×5 scan result centered at `center`.
    Returns the number of new cells discovered.
    """
    ...
    # Handle different scan sizes gracefully
    scan_size = len(scan)
    if scan_size == 0:
        return 0
    ...
    for i, row in enumerate(scan):
        ...
        for j, symbol in enumerate(row):
            ...
            if symbol == "X":
                continue  # out of bounds marker
                
            if within_bounds(x, y, L):
                old_val = known.get((x, y))
                if symbol == "W":
                    add_cell(known, (x, y), "W")
                    if old_val != "W":
                        new_discoveries += 1
                elif symbol in ("_", "C"):  # empty or crow
...
def scan_would_reveal(center: Tuple[int, int], L: int,
                      known: Dict[Tuple[int, int], str], min_unknown: int = 1) -> bool:
    """
    Return True if a scan at `center` would reveal at least `min_unknown` unknown
    in-bounds cells within the scan radius.
    """
    x0, y0 = center
    unknown_count = 0
...
def compute_scan_positions(L: int) -> List[Tuple[int, int]]:
    """
    Compute scanning positions with better coverage.
    Uses overlapping pattern for better efficiency.
    """
    positions = []
    
    # For very small grids
    if L <= 5:
        positions.append((L // 2, L // 2))
        return positions
    
    # Use step size of 4 instead of 5 for better overlap
    step = 4
    start_offset = SCAN_RADIUS
    ...
    for y in [1, L - 2]:
        if 0 <= y < L:
            for x in range(start_offset, L, step):
                if (x, y) not in positions:
                    edge_positions.append((x, y))
    
    for x in [1, L - 2]:
        if 0 <= x < L:
            for y in range(start_offset, L, step):
                if (x, y) not in positions:
                    edge_positions.append((x, y))
...
def find_path_bfs(start: Tuple[int, int], goal: Tuple[int, int], L: int,
                  known: Dict[Tuple[int, int], str], avoid_crows: Set[Tuple[int, int]] = None) -> List[str]:
    """
    Find complete path from start to goal using BFS.
    Returns list of direction strings, or empty list if no path.
    """
    if start == goal:
        return []
    
    if avoid_crows is None:
        avoid_crows = set()
    ...
    while queue:
        ...
        for direction in DIR_ORDER:
            ...
            if not within_bounds(nx, ny, L):
                continue
            if known.get(next_pos) == "W":
                continue
            if next_pos in avoid_crows:
                continue
...
def get_next_step(start: Tuple[int, int], goal: Tuple[int, int], L: int,
                  known: Dict[Tuple[int, int], str], avoid_crows: Set[Tuple[int, int]] = None) -> Optional[str]:
    """Get the first step of the shortest path from start to goal."""
    path = find_path_bfs(start, goal, L, known, avoid_crows)
    return path[0] if path else None


def find_frontier_cells(known: Dict[Tuple[int, int], str], L: int) -> Set[Tuple[int, int]]:
    """Find all unknown cells adjacent to known cells (frontier)."""
    frontier = set()
    
    for (x, y), cell_type in known.items():
        if cell_type in ("W", "E"):
            # Check adjacent cells
            for dx, dy in DIRS.values():
                nx, ny = x + dx, y + dy
                if within_bounds(nx, ny, L) and (nx, ny) not in known:
                    frontier.add((nx, ny))
    
    return frontier


def find_best_scan_position(crows: Dict[str, Tuple[int, int]], known: Dict[Tuple[int, int], str], 
                           L: int, frontier: Set[Tuple[int, int]]) -> Optional[Tuple[int, int]]:
    """Find the best position to scan based on frontier cells."""
    if not frontier:
        return None
    ...
    # Consider positions that would cover multiple frontier cells
    for fx, fy in frontier:
        for dx in range(-SCAN_RADIUS, SCAN_RADIUS + 1):
            for dy in range(-SCAN_RADIUS, SCAN_RADIUS + 1):
                scan_x, scan_y = fx + dx, fy + dy
                scan_pos = (scan_x, scan_y)
                
                if not within_bounds(scan_x, scan_y, L):
                    continue
                if known.get(scan_pos) == "W":
                    continue
                
                # Count how many frontier cells this scan would cover
                score = 0
                for sx in range(scan_x - SCAN_RADIUS, scan_x + SCAN_RADIUS + 1):
                    for sy in range(scan_y - SCAN_RADIUS, scan_y + SCAN_RADIUS + 1):
                        if (sx, sy) in frontier:
                            score += 1
...
def assign_tasks(crows: Dict[str, Tuple[int, int]], targets: Set[Tuple[int, int]], 
                known: Dict[Tuple[int, int], str], L: int) -> Dict[str, Tuple[int, int]]:
    """Assign targets to crows using Hungarian-like algorithm (greedy approximation)."""
    if not targets:
        return {}
    ...
    # Sort crows by ID for deterministic behavior
    sorted_crows = sorted(crows.items())
    
    for crow_id, crow_pos in sorted_crows:
        ...
        for target in available_targets:
            distance = manhattan_distance(crow_pos, target)
            if distance < best_distance:
                best_distance = distance
                best_target = target
        
        if best_target:
            assignments[crow_id] = best_target
...
def decide_action(state: Dict) -> Dict:
    """
    Improved decision making with multiple strategies.
    """
    L = state["grid_size"]
    known = state["known"]
    crows = state["crows"]
    ...
    # Count discovered walls
    walls_found = sum(1 for v in known.values() if v == "W")
    
    # If we found all walls, submit
    if walls_found >= num_walls:
        return {"action_type": "submit"}
    
    # Get other crow positions to avoid collisions
    crow_positions = set(crows.values())
    
    # Strategy 1: Check if any crow can perform a useful scan at current position
    for cid, pos in crows.items():
        if scan_would_reveal(pos, L, known, MIN_USEFUL_UNKNOWN):
            logger.info(f"Crow {cid} scanning at current position {pos}")
            return {"action_type": "scan", "crow_id": cid}
    
    # Strategy 2: Use planned scanning centers
    if "scan_centres_to_scan" in state and state["scan_centres_to_scan"]:
        centres_to_scan = state["scan_centres_to_scan"]
        
        # Remove centers that no longer need scanning
        centres_to_remove = []
        for centre in centres_to_scan:
            if not scan_would_reveal(centre, L, known, MIN_USEFUL_UNKNOWN):
                centres_to_remove.append(centre)
        
        for centre in centres_to_remove:
            centres_to_scan.remove(centre)
            state["scan_centres_done"].add(centre)
        
        # If we still have centers to scan
        if centres_to_scan:
            # Check if any crow is at a scan center
            for cid, pos in crows.items():
                if pos in centres_to_scan:
                    logger.info(f"Crow {cid} scanning at planned center {pos}")
                    return {"action_type": "scan", "crow_id": cid}
            
            # Assign crows to nearest scan centers
            assignments = assign_tasks(crows, centres_to_scan, known, L)
            
            if assignments:
                # Find crow with shortest path to its target
                best_crow = None
                best_distance = float('inf')
                
                for crow_id, target in assignments.items():
                    crow_pos = crows[crow_id]
                    distance = manhattan_distance(crow_pos, target)
                    if distance < best_distance:
                        best_distance = distance
                        best_crow = (crow_id, target)
                
                if best_crow:
                    crow_id, target = best_crow
                    crow_pos = crows[crow_id]
                    other_crows = crow_positions - {crow_pos}
                    
                    step = get_next_step(crow_pos, target, L, known, other_crows)
                    if step:
                        logger.info(f"Crow {crow_id} moving {step} toward scan center {target}")
                        return {"action_type": "move", "crow_id": crow_id, "direction": step}
    
    # Strategy 3: Frontier-based exploration
    frontier = find_frontier_cells(known, L)
    
    if frontier:
        best_scan_pos = find_best_scan_position(crows, known, L, frontier)
        
        if best_scan_pos:
            # Find crow that can reach the best scan position most efficiently
            best_crow = None
            best_distance = float('inf')
            
            for crow_id, crow_pos in crows.items():
                distance = manhattan_distance(crow_pos, best_scan_pos)
                if distance < best_distance:
                    best_distance = distance
                    best_crow = crow_id
            
            if best_crow:
                crow_pos = crows[best_crow]
                
                if crow_pos == best_scan_pos:
                    logger.info(f"Crow {best_crow} scanning at frontier position {best_scan_pos}")
                    return {"action_type": "scan", "crow_id": best_crow}
                else:
                    other_crows = crow_positions - {crow_pos}
                    step = get_next_step(crow_pos, best_scan_pos, L, known, other_crows)
                    if step:
                        logger.info(f"Crow {best_crow} moving {step} toward frontier scan position {best_scan_pos}")
                        return {"action_type": "move", "crow_id": best_crow, "direction": step}
    
    # Strategy 4: Random exploration if stuck
    unknown_cells = []
    for y in range(L):
        for x in range(L):
            if (x, y) not in known:
    ...
    if unknown_cells:
        # Pick a random unknown cell and move toward it
        target_cell = unknown_cells[0]  # Could randomize this
        
        best_crow = None
        best_distance = float('inf')
        
        for crow_id, crow_pos in crows.items():
            distance = manhattan_distance(crow_pos, target_cell)
            if distance < best_distance:
                best_distance = distance
                best_crow = crow_id
        ...
        if best_crow:
            crow_pos = crows[best_crow]
            other_crows = crow_positions - {crow_pos}
            step = get_next_step(crow_pos, target_cell, L, known, other_crows)
            if step:
                logger.info(f"Crow {best_crow} exploring toward unknown cell {target_cell}")
                return {"action_type": "move", "crow_id": best_crow, "direction": step}
    
    # Fallback: scan with any crow
    any_crow = next(iter(crows.keys()))
    logger.info(f"Fallback: scanning with crow {any_crow}")
    return {"action_type": "scan", "crow_id": any_crow}
...
def fog_of_wall():
    """
    Improved endpoint for the Fog-of-Wall game with better error handling
    and more robust strategy selection.
    """
    try:
        ...
        if key not in STATE and payload.get("test_case"):
            ...
            if not crows:
                return jsonify({"error": "No valid crows found"}), 400
            
            # Initialize known map
            known = {}
            for pos in crows.values():
                add_cell(known, pos, "E")
            
            # Compute scanning centers
            scan_centres = compute_scan_positions(L)
            ...
            STATE[key] = {
                ...
                "targets": {},
                "move_count": 0,
                "last_positions": defaultdict(list),  # Track positions to detect stuck crows
            }
            logger.info(f"Initialized game {game_id} with grid size {L}, {num_walls} walls, {len(crows)} crows")

        if key not in STATE:
            return jsonify({"error": "Unknown game. Send initial test_case first."}), 400
        ...
        if prev:
            ...
            if act == "move" and cid in crows:
                ...
                if mr is not None:
                    new_pos = mr
                    if new_pos == old_pos and direction in DIRS:
                        # Hit a wall
                        dx, dy = DIRS[direction]
                        wall_x, wall_y = old_pos[0] + dx, old_pos[1] + dy
                        if within_bounds(wall_x, wall_y, L):
                            add_cell(known, (wall_x, wall_y), "W")
                            logger.info(f"Discovered wall at {wall_x}, {wall_y}")
                    else:
                        # Successful move
                        crows[cid] = new_pos
                        ...
                        state["last_positions"][cid].append(new_pos)
                        if len(state["last_positions"][cid]) > MAX_STUCK_MOVES:
                            state["last_positions"][cid].pop(0)
            
            elif act == "scan" and cid in crows:
                scan_result = prev.get("scan_result")
                if scan_result:
                    new_discoveries = update_known_from_scan(crows[cid], scan_result, L, known)
                    logger.info(f"Scan at {crows[cid]} revealed {new_discoveries} new cells")
                    
                    # Mark scan center as done if we were at one
                    pos = crows[cid]
                    if pos in state.get("scan_centres_to_scan", set()):
        ...
        if decision.get("action_type") == "submit":
            ...
            logger.info(f"Submitting {len(submission)}/{state['num_walls']} walls after {state['move_count']} moves")
            # Clean up state
            STATE.pop(key, None)
        else:
            resp.update({
            ...
            if decision["action_type"] == "move":
                resp["direction"] = decision["direction"]
        
        return jsonify(resp)
        
    except Exception as e:
        logger.error(f"Error in fog_of_wall: {e}", exc_info=True)
